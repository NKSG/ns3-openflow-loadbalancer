...
void RoundRobinController::ReceiveFromSwitch(Ptr<OpenFlowSwitchNetDevice> swtch, ofpbuf* buffer) {
	...
	if (type == OFPT_PACKET_IN) // The switch didn't understand the packet it received, so it forwarded it to the controller.
	{
		...
		uint32_t flow_nw_src = ntohl (key.flow.nw_src);
		uint32_t flow_nw_dst = ntohl (key.flow.nw_dst);
		Ipv4Address src_ipv4Addr (flow_nw_src);
		Ipv4Address dst_ipv4Addr (flow_nw_dst);
		Ipv4Address server_ipv4Addr ("10.1.1.254");  //TODO:

		NS_LOG_LOGIC (src_ipv4Addr);
		NS_LOG_LOGIC (dst_ipv4Addr);

		if (!dst_ipv4Addr.IsBroadcast()) {
			NS_LOG_LOGIC ("Branch in : Not broadcast");
			if (dst_ipv4Addr.IsEqual(server_ipv4Addr)) {
				NS_LOG_LOGIC ("Branch in : flow from client to server");
				// Add or update record of last used port, server address as key
				RoundRobinState_t::iterator iter = m_lastState.find(dst_ipv4Addr);
				if (iter != m_lastState.end()) {
					out_port = iter->second.port;
					int server_number = OF_DEFAULT_SERVER_NUMBER;  //TODO:
					...
					RoundRobinState rrs;
					rrs.port = out_port;
					m_lastState.insert (std::make_pair (dst_ipv4Addr, rrs)); // insert new record
				}

				// Record this client to server flow in_port, client address as key
				PortRecord_t::iterator st = m_portrecord.find (flow_nw_src);
				if (st == m_portrecord.end ()) {
				   PortRecord pr;
				   pr.port = in_port;
				   m_portrecord.insert (std::make_pair (flow_nw_src,pr));
				}

			} else if (src_ipv4Addr.IsEqual(server_ipv4Addr)) {
				NS_LOG_LOGIC ("Branch in : flow from server to client");
				// Check recorded client addresses for out_port
				PortRecord_t::iterator st = m_portrecord.find (flow_nw_src);
				if (st != m_portrecord.end ()) {
					out_port = st->second.port;
				}
			} else {
				NS_LOG_LOGIC ("Branch in : flow don't know...");
			}
		} else {
